...
public class AccountDAO implements DAO<Account> {
    ...
    public void insert(Account account) {
        try (PreparedStatement preparedStatement = dataSourceService.getPreparedStatement(AccountQuerier.INSERT_ACCOUNT_INFO)) {
            ...
            log.error("Ошибка подключения к БД при попытке вставки записи с данными счета" + AccountQuerier.INSERT_ACCOUNT_INFO, e);
        } finally {
    ...
    public Account get(String firstParam) throws Exception {
        ...
        try (PreparedStatement preparedStatement = dataSourceService.getPreparedStatement(AccountQuerier.SELECT_ACCOUNT_BY_ACCOUNT_NUMBER)) {
            ...
            if (resultSet.next()) {
                ...
                throw new UnregistredAccountException("Cчет с номером " + accountNumber + " отсутствует.");
            }
        } catch (DataSourceServiceException e) {
            log.error("Ошибка при получении данных о счете с номером " + accountNumber + " отсутствует");
            return null;
        } catch (SQLException e) {
    ...
    public List<Account> getAll() {
        ...
        return allAccountNumbers;
            ...
            log.error("Ошибка при получении саисков всех номеров счетов", e);
            return null;
        } catch (SQLException e) {
